            ...
            if (message.direct() == Direct.SEND) {
                ...
                if (!TextUtils.isEmpty(avatar)) {
                    CommonGlideImageLoader.getInstance()
                            .displayNetImageWithCircle(context,avatar,userAvatarView,getResources().getDrawable(R.drawable.ease_default_avatar));
                } else {
                    Glide.with(context).load(R.drawable.ease_default_avatar).into(userAvatarView);
                }
                usernickView.setText(message.getStringAttribute("nickName", message.getUserName()));
            }
        }
